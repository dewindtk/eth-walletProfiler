...
const kaikun = utils.kaikun
const kaikunTxs = require('./kaikunTxs/kaikunMergedSlicedTxs.json')
var web3 = new Web3(Web3.givenProvider || 'https://eth-mainnet.alchemyapi.io/v2/JNemY7zxf_s_VMtUSeb74DsUYIyuzedA');

ETHInv = 0.0 //To check: where is this value stored?
ERC20Inv = {}
ERC721Inv = {}
ERC1155Inv = {}
...
{
    if (tx.to.toLowerCase() === kaikun && tx.isError === "0")
    {
        ETHInv += parseFloat(ethers.utils.formatEther(tx.value))
    }
}
...
function processERC20(tx)
{
    ...
    if (tx.to.toLowerCase() === kaikun)  //Emit errors TODO
    {
    ...
    if (tx.from.toLowerCase() === kaikun) //Emit errors TODO 
    {
...
function processERC721(tx)
{
    ...
    if ((tx.contractAddress === "0x7d9d3659dcfbea08a87777c52020bc672deece13" || 
        ...
        tx.to.toLowerCase() === kaikun && tx.from.toLowerCase() === kaikun)
    {
...
async function main() 
{
    ...
    txs = utils.txIterator(kaikunTxs)
    done = false
    ...
    while(1)
    {
        ...
        // console.log("Total Value: ", ETHInv)
        // console.log("ERC20 Inven: ", ERC20Inv)
        // console.log("ERC721 Inve: ", ERC721Inv)
        // console.log("ERC1155 Inv: ", ERC1155Inv)
